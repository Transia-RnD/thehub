...
> `go/build [username] [name] [version | latest] [production | standalonestandalone]`

- `go/build transia icv2 production` to build the production container image (tag: `rippled`)
- `go/build transia icv2 standalonestandalone` to build the standalonestandalone container image (tag: `rippled`)
...
`docker build --tag transia/icv2:latest --target=standalonestandalone .`
