...
docker run -dit \
    --name rippled \
    -p 80:80 \
    -v /my/local/disk/testnet-config/:/config/ \
    transia/testnet:latest
```
...
- **????-??-??** [version] [is released]()
- **2022-08-25** latest [is released](https://github.com/Transia-RnD/thehub/tree/main/amendments/testnet)

## Update process

1. Stop the container: `docker stop rippled` (if you named (`--name`) the container `rippled`)
2. Remove the container: `docker rm rippled`
3. Remove the image: `docker rmi transia/testnet:latest` (or if you built the container image based on the [Github repo](https://github.com/WietseWind/docker-rippled): use the image name you specified when building)
...
docker build --no-cache --tag transia/testnet:$version .
docker push transia/testnet:$version
docker tag transia/testnet:$version transia/testnet:latest
docker push transia/testnet:latest
```