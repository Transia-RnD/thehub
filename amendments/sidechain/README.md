...
- `go/build transia sidechain production` to build the production container image (tag: `rippled`)
- `go/build transia sidechain genesis` to build the genesis container image (tag: `rippled`)
- `go/up transia sidechain` to create a new container named `rippled` and setup the port and persistent config (*1)
- `go/down transia sidechain` to stop and remove the container `rippled`
...
`docker build --tag transia/sidechain:latest --target=genesis .`

### From the Docker Hub

Use the image `transia/sidechain`.
...
docker run -dit \
    --name rippled \
    -p 80:80 \
    -v /my/local/disk/sidechain-config/:/config/ \
    transia/sidechain:latest
```
...
- **????-??-??** [version] [is released]()
- **2022-08-25** latest [is released](https://github.com/Transia-RnD/thehub/tree/main/amendments/sidechain)

## Update process

1. Stop the container: `docker stop rippled` (if you named (`--name`) the container `rippled`)
2. Remove the container: `docker rm rippled`
3. Remove the image: `docker rmi transia/sidechain:latest` (or if you built the container image based on the [Github repo](https://github.com/WietseWind/docker-rippled): use the image name you specified when building)
...
docker build --no-cache --tag transia/sidechain:$version .
docker push transia/sidechain:$version
docker tag transia/sidechain:$version transia/sidechain:latest
docker push transia/sidechain:latest
```